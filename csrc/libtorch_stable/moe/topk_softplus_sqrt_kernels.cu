...
__launch_bounds__(128) __global__
  ...
  if (lane < 6) {
    ...
    indices[offset] = static_cast<IndType>(expert);
  }
}

template <typename IndType, typename InputIdType = IndType>
void launchDsv4HashTopk(const float* input, float* output, IndType* indices,
                        int num_rows, int num_experts,
                        double routed_scaling_factor,
                        const InputIdType* input_ids, const IndType* tid2eid,
                        cudaStream_t stream) {
  if (num_rows == 0) return;
  auto* kernel = &dsv4HashTopkSoftplusSqrt<IndType, InputIdType>;
...
#ifndef USE_ROCM
  if constexpr (std::is_same_v<InputType, float> &&
                std::is_same_v<IndType, HashIndType>) {
...
void dispatch_topk_softplus_sqrt_launch(
  ...
  if constexpr (std::is_same_v<ComputeType, float>) {
    if (input_ids.has_value() && tid2eid.has_value() && topk == 6 &&
        renormalize && (num_experts == 256 || num_experts == 384) &&
        topk_indices.scalar_type() == torch::headeronly::ScalarType::Long &&
        input_ids.value().scalar_type() == torch::headeronly::ScalarType::Int &&
        tid2eid.value().scalar_type() == torch::headeronly::ScalarType::Long) {
      vllm::moe::launchDsv4HashTopk<int64_t, int>(
          gating_output, topk_weights.mutable_data_ptr<float>(),
          topk_indices.mutable_data_ptr<int64_t>(), num_tokens, num_experts,
          routed_scaling_factor, input_ids.value().const_data_ptr<int>(),
